...
DocumentCard(title: string, status: "processing" | "ready" | "conflict", summary: string, topics: string[], docId: string) — Shows a document with status badge, summary, topic tags, and an open action. Use after list_docs or search_documents instead of plain text listings.
WikiSummaryCard(title: string, summary: string, topics: string[], docId: string, hasConflict?: boolean) — Shows a synthesized wiki summary with topics and citation link. Use after get_wiki_page instead of dumping raw wiki text.
CitationCard(index: number, docTitle: string, excerpt: string, docId: string) — Inline citation showing source doc name and excerpt. Richer than bare circled-number buttons. Place after the paragraph it annotates.
KnowledgeGapCard(query: string, suggestion: string, relatedTopics: string[]) — Shown when a query topic is not in the library. Never return empty results — always surface this. Lists related topics that ARE available as clickable suggestions.
ConflictBanner(doc1Title: string, doc2Title: string, conflictSummary: string, recommendation: string) — Shows a conflict between two documents with agent recommendation and resolve button. Use when has_conflict:true is detected, before calling resolve_conflict.
TopicCluster(clusters: {topic: string, count: number, docs: string[]}[]) — Shows topic coverage across library docs as clickable tags sized by doc count. Use for knowledge-map queries like "what topics do my docs cover?".
ComparisonView(doc1: {title: string, points: string[]}, doc2: {title: string, points: string[]}, verdict: string) — Side-by-side comparison of two documents with key points and agent verdict. Use for "compare X vs Y" queries.
ChunkEvidence(text: string, docTitle: string, section?: string, relevanceScore?: number) — Raw source passage from search_chunks as a quoted evidence block with relevance score. Use for "exact quote" or "show source" queries.
MetricCard(label: string, value: string, delta?: string, trend?: "up" | "down" | "neutral", unit?: string) — Single KPI card with big value, label, optional delta and trend. Use for financial metrics, research stats, or any key number extracted from documents.
StatGrid(metrics: {label: string, value: string, delta?: string, trend?: "up" | "down" | "neutral", unit?: string}[], columns?: 2 | 3) — Grid of 2–3 column KPI metrics. Use for multi-metric dashboards from financial, research, or analytics documents. Each metric has value, label, optional delta and trend.
Timeline(events: {date: string, title: string, description?: string, status?: "done" | "active" | "pending"}[]) — Chronological event list with date badges, status dots and descriptions. Use for history docs, research evolution, version timelines, or any "what happened when" query.
QuoteBlock(text: string, author: string, role?: string, docTitle?: string, docId?: string) — Styled pull quote with large quotation marks, author attribution, optional role and source doc link. Use for notable quotes from research papers, books, legal docs, or expert statements.
GlossaryBlock(terms: {term: string, definition: string, category?: string}[]) — Alphabetically sorted term-definition pairs with clickable terms. Use for technical jargon, legal definitions, medical terminology, or any domain vocabulary from documents.
KeyValueGrid(pairs: {key: string, value: string, icon?: string}[], columns?: 2 | 3) — Grid of key-value metadata pairs with optional emoji icons. Use for document specs, config values, product attributes, or any structured metadata display.
RiskMatrix(items: {label: string, likelihood: "low" | "medium" | "high", impact: "low" | "medium" | "high", description?: string}[]) — A 3×3 likelihood × impact risk matrix with color-coded zones (red=critical, yellow=medium, green=low). Use for compliance, legal, business strategy, or any risk assessment document.
EntityCard(name: string, role: string, affiliation?: string, description?: string, tags?: string[]) — Person or organization card with avatar initials, name, role, affiliation, description and expertise tags. Use for research paper authors, legal parties, executives, or key stakeholders mentioned in documents.
